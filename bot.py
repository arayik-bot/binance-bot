...
def _ta_text(coin: str, price: float, ta: dict, iv: str) -> str:
    ...
    return (
        ...
        f"   {'Oversold' if ta['rsi'] < 30 else ('Overbought' if ta['rsi'] > 70 else 'Normal')}\n\n"
...
async def do_auto_trade(uid: int, chat_id: int, coin: str,
    ...
    text = (
        ...
        f"Yntrec chap kam spasec {AUTO_CONFIRM_TIMEOUT}v:"
    )
    rows = []
    row  = []
    for s in TRADE_SIZES:
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    mock_note = "\n⚠️ _Demo mode_" if not bc else \
                ("\n🟡 _Testnet mode_" if USE_TESTNET else "\n🟢 _LIVE trading_")
    text = (
        f"👋 *Bariv galusts, {name}!*\n\n"
        f"🤖 *Binance Pro Trading Bot*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🛒 Buy / 💰 Sell manually\n"
        f"🤖 Auto-trade + confirmation\n"
        f"⏳ {AUTO_CONFIRM_TIMEOUT}s no response = auto execute\n"
        f"💵 Sizes: $5 ~ $50\n"
        f"🪙 15 coins: {', '.join(TOP_COINS[:8])}...\n"
        f"{mock_note}\n\n"
        f"👇 Choose action:"
    )
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    text = (
        "📚 *COMMANDS*\n\n"
        "`/start` — Main menu\n"
        "`/buy BTC 20` — Buy $20 BTC\n"
        "`/sell ETH 15` — Sell $15 ETH\n"
        "`/auto BTC` — Auto signal\n"
        "`/scan` — Scan all coins\n"
        "`/price BTC ETH` — Prices\n"
        "`/portfolio` — Portfolio + PnL\n"
        "`/analysis BTC` — TA indicators\n"
...
async def cmd_auto(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    args = ctx.args
    if not args:
        await update.message.reply_text("📌 `/auto BTC`", parse_mode=ParseMode.MARKDOWN)
        return
...
async def cmd_alert(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    USER_DATA[uid]["alerts"].append(
        {"symbol": sym(s), "condition": cond, "price": price,
         "chat_id": update.effective_chat.id})
    await update.message.reply_text(
        f"🔔 Alert set: *{sym(s)}* {'⬆️' if cond == 'above' else '⬇️'} `${price:,.2f}`",
        parse_mode=ParseMode.MARKDOWN)
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "m_main":
        ...
        await q.edit_message_text(
            "🤖 *Auto-Trade — Select coin:*",
            parse_mode=ParseMode.MARKDOWN, reply_markup=auto_trade_kb())
