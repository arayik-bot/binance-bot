    ...
    if bc:
        try:
            ...
            if "1003" in str(e) or "banned" in str(e).lower():
                log.warning("Binance rate limit hit, using cache/mock")
                if s in _price_cache:
                    return _price_cache[s][0]
...
async def auto_job(ctx):
    for uid,data in list(USER_DATA.items()):
        ...
        if has_usdt:
            ...
            # Only scan held coins + 3 random ones to save API calls
            scan_coins = list(set(held_coins[:5] + random.sample(coins, min(3, len(coins)))))
